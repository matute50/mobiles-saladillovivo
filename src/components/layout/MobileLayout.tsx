...
// MobileNewsCard: Fondo del botón Play ajustado a la política de color
      ...
      <div className="relative w-full h-full">
        ...
        <div className="absolute inset-0 z-20 flex flex-col">
            <div className={cn("flex-1 flex items-center justify-center min-h-0", isFeatured && "pt-16")}>
                {/* CAMBIO: Fondo dinámico usando #6699ff (oscuro) y #003399 (claro) */}
                <div className={cn("flex items-center justify-center rounded-full backdrop-blur-sm border border-white/20 shadow-2xl transition-transform duration-300 group-active:scale-90", isDark ? "bg-[#6699ff]/60" : "bg-[#003399]/60", isFeatured ? "p-4" : "p-3", isFeatured && "-translate-y-[20px]")}>
...
// VideoCarouselBlock: Flechas y Títulos ajustados a la política de color
  ...
  return (
    <div className="flex flex-col gap-0 h-full w-full">
      <div className="flex items-center justify-between px-2 py-0.5 shrink-0 rounded-lg mx-1 transition-colors bg-transparent">
        {/* Flecha Izquierda: #6699ff (oscuro) / #003399 (claro) */}
        ...
        {/* Título Categoría: #6699ff (oscuro) / #003399 (claro) */}
        ...
        {/* Flecha Derecha: #6699ff (oscuro) / #003399 (claro) */}
               ...
               <div onClick={() => handlePlay(video)} className={cn("relative h-full w-full rounded-lg overflow-hidden border active:scale-95 transition-transform group", isDark ? "bg-neutral-800 border-neutral-700/50" : "bg-white border-neutral-200 shadow-sm")}>
                  <Image src={video.imagen || getYouTubeThumbnail(video.url)} alt={video.nombre} fill className="object-cover opacity-90 group-hover:opacity-100 transition-opacity" sizes="150px" />
                   <div className="absolute inset-0 bg-black/10 flex items-center justify-center">
                      {/* Play del video (para consistencia): #6699ff (oscuro) / #003399 (claro) */}
                      <div className={cn("p-1.5 rounded-full backdrop-blur-sm border border-white/10", "-mt-[50px]", isDark ? "bg-[#6699ff]/60" : "bg-[#003399]/60")}> <Play size={21} fill="white" className="text-white" /> </div>
                   </div>
      ...
      <div className={cn("fixed inset-0 z-[100] flex flex-col w-full h-[100dvh] overflow-hidden transition-colors duration-300", themeClasses.bg, themeClasses.text)}>
        ...
        <div className="flex-1 flex flex-col gap-2 px-3 pt-[2px] pb-1 min-h-0">
          <div className="w-full px-1 mt-1 text-center shrink-0">
             {/* TITULO ULTIMAS NOTICIAS - POLITICA DE COLOR APLICADA */}
