...
// Configuración de Memoria Local
const VIEWED_HISTORY_KEY = 'sv_viewed_history_v2';
    ...
    <div onClick={onClick} className={cn("relative overflow-hidden rounded-xl shadow-sm shrink-0 active:scale-[0.98] transition-transform", isDark ? "bg-neutral-900 border border-neutral-800" : "bg-white border border-neutral-200", isFeatured ? "w-full h-full" : "w-[49%] h-full")}>
      <div className="relative w-full h-full">
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-center items-center p-3 text-center">
            ...
            {/* INTERLINEADO AMPLIADO A leading-[1.1] */}
            <h3 className={cn("text-white font-black uppercase tracking-tight leading-[1.1] text-balance", isFeatured ? "text-2xl line-clamp-3" : "text-[13px] line-clamp-4")}>{news.titulo}</h3>
        </div>
      </div>
    </div>
  );
...
export default function MobileLayout({ data }: { data: PageData }) {
  ...
  const { playManual, setVideoPool, state, handleContentEnded } = useMediaPlayer(); 
  ...
  // LÓGICA DE MEMORIA DE 7 DÍAS
  const markAsViewed = useCallback((id: string) => {
    const saved = localStorage.getItem(VIEWED_HISTORY_KEY);
    let history = saved ? JSON.parse(saved) : [];
    const now = Date.now();
    history = history.filter((item: any) => item.id !== id && (now - item.timestamp) < SEVEN_DAYS_MS);
  ...
  const getFilteredPool = useCallback(() => {
    ...
    const now = Date.now();
    const viewedIds = history.filter((item: any) => (now - item.timestamp) < SEVEN_DAYS_MS).map((i: any) => i.id);
    const notViewed = data.videos.allVideos.filter(v => !viewedIds.includes(v.id));
    return notViewed.length > 0 ? notViewed : data.videos.allVideos;
  }, [data]);

  // Sobrescribir el fin de contenido para marcar como visto y elegir siguiente
  const onContentEndedCustom = useCallback(() => {
    if (state.currentContent?.id) markAsViewed(state.currentContent.id);
    const pool = getFilteredPool();
    const next = pool[Math.floor(Math.random() * pool.length)];
    playManual(next);
  }, [state.currentContent, markAsViewed, getFilteredPool, playManual]);
  ...
  return (
    <div className={cn("fixed inset-0 flex flex-col w-full h-[100dvh] overflow-hidden transition-colors", isDark ? "bg-black" : "bg-neutral-50")}>
             ...
             {infoModal.view === 'decreto' ? (
               ...
               <div className="flex flex-col items-center w-full pt-4 text-center">
                  ...
                  <div className={cn("space-y-4 text-[14px] leading-relaxed px-2", isDark ? "text-neutral-300" : "text-neutral-700")}>
                    ...
                    <p className="font-black italic py-2 border-y border-neutral-500/20 text-pretty">"Nunca fue mi intención poner a funcionar una plataforma más, sino crear identidad."</p>
      ...
      {/* REPRODUCTOR SIN BORDES NI SOMBRAS */}
