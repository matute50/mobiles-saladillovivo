...
// vFINAL: Implementamos Cache (ISR 60s) para reducir carga en DB y mejorar velocidad
export const getPageData = unstable_cache(
    ...
    try {
      const [articlesRes, videosRes, adsRes] = await Promise.all([
        ...
        supabase
          ...
          .limit(10), // Limitado a 100 para no sobrecargar el cache en mobile (antes 5000 era excesivo)
        supabase
          .from('anuncios')
          .select('*')
      ]);
  ...
  ['page-data'], // Tag de cache para revalidación manual si fuera necesario
  { revalidate: 60, tags: ['content'] } // Revalidar cada 60 segundos
);
